...
Web application framework is a software framework that is designed to 
...
class HttpError(Exception):
	"""
	HttpError that defines http errpr code.
	...
	def header(self, name, value):
		if not hasattr(self, '_headers'):
			self._header = [_HEADER_X_POWERED_BY]
...
class RedirectError(HttpError):
	...
	>>> e = RedirectError(302, 'htto://www.apple.com/')
	>>> e.status
	'302 Found'
	>>> e.location
	'http://www.apple.com/'
...
def internalerror():
	"""
	Send an internalerror response.

	>>> raise internalerror()
	Traceback (most recent call last):
		...
	HttpError: 500 Internalerror
	"""
...
class Request(object):
	...
	def __getitem__(self, key):
		"""
		Get input parameter value. If the specified key has multiple value, the first ont is returned.
		...
		Traceback (most recent call last):
			...
		KeyError: 'empty'
		>>> b = '----WebKitFromBoundaryQQ3J8kPsjFpTmqNz'
	...
	def headers(self):
		"""
		Get all HTTP headers with key as str and values as unicode. The header names are 'XXX-XXX' uppercase.
		"""
...
class Response(object):
	...
	def content_type(self, value):
		...
		if value:
			...
			self.unset_header('CONTENT_TYPE')
	...
	def status(self, value):
		...
		if isinstance(value, (int, long)):
			...
			if _RE_RESPONSE_STATUE.match(value):
				self._status = value
			else:
...
class WSGIApplication(object):
	...
	def get_wsgi_application(self, debug=False):
		...
		fn_exec = _build_interceptor_chain(fn_route, *self.__interceptors)
